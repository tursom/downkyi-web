...
- 解析和重试实时显示阶段、已处理数量、成功／失败数量、当前条目和耗时；总数明确后显示进度条，总数未知时不估算百分比。
- 第一步只读取标题、分 P／剧集列表等元信息。单项自动解析资源；多项默认不选，由用户选择后再解析所选视频的画质，未选择的视频不会请求播放资源。
...
3. 点击新建下载，粘贴链接读取视频列表。普通视频若属于 UGC 合集，会列出所属合集；分 P 视频则列出分 P，显式 `?p=` 链接只列出指定 P。此时仅获取列表元信息，不逐个请求画质和播放地址。
4. 列表只有一项时自动解析资源；多项默认不勾选，手动选择后点击「解析所选项目」。每次最多选择 50 项。未选条目保留“待解析”状态，不能直接入队；返回列表可继续选择其他项目。列表最多 100 项，超过时显示截断提示，解析记录保存一小时；每次网络解析最长 180 秒。
5. 对解析失败的条目，可点击该条目的「重试」，或使用列表上方的「重新解析失败项」。只重试失败项，保留已有勾选和下载规格；恢复的条目可手动勾选。重试可以取消，失败或超时不会清空列表。原解析记录超过一小时后，需要重新解析原链接。临时请求受限以警示图标显示。
6. 选择项目、画质、编码和附件，检查确认页后加入队列。每批最多 50 个任务；未完成队列最多 500 个。
7. 在任务表格里暂停/继续/重试。暂停保留 `.part` 等续传文件；如果上游支持恢复，重试会利用保留的数据。
8. 在文件库中查看服务器真实文件并下载到当前电脑。下载目录按任务 ID 隔离，内部使用稳定的 `media.ext` 文件名保证重试续传；网页显示对应的视频标题，避免不同任务覆盖。
