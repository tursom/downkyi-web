...
- Parse and retry support optional NDJSON progress without changing default JSON
  responses. Progress is generated by actual worker stages and completed entries;
  unknown playlist cardinality stays null rather than estimating percentages.
- UI shows stage, processed/success/failure counts, current title and elapsed time.
  Known totals (including selected retry batches) use a determinate progress bar.
  Cancel, timeout and stale-response handling preserve existing wizard behavior.
- Backend: 280 tests passed. Frontend: 104 tests passed; TypeScript/Vite build and
  110 fixture browser checks at four viewport widths passed.
- Real collection verification emitted 121 progress events, first at about
  0.06 seconds, and completed in about 140 seconds. All 59 entries succeeded,
  including BV1HqL16BEeF. Completed counts 1–59 arrived before the terminal
  result; final counts matched it. No download tasks were created.
- Both HTTP and worker stream readers handle split records and enforce size
  limits. Authentication/input/cache/concurrency errors still precede streaming;
  disconnect and timeout tests verify cleanup of private snapshots/process groups.
