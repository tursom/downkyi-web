...
4. 对解析失败的条目，可点击该条目的「重试」，或使用列表上方的「重新解析失败项」。只重试失败项，保留已有勾选和下载规格；恢复的条目可手动勾选。重试可以取消，失败或超时不会清空列表。原解析记录超过一小时后，需要重新解析原链接。临时请求受限以警示图标显示。
5. 选择项目、画质、编码和附件，检查确认页后加入队列。每批最多 50 个任务；未完成队列最多 500 个。
6. 在任务表格里暂停/继续/重试。暂停保留 `.part` 等续传文件；如果上游支持恢复，重试会利用保留的数据。
7. 在文件库中查看服务器真实文件并下载到当前电脑。下载目录按任务 ID 隔离，内部使用稳定的 `media.ext` 文件名保证重试续传；网页显示对应的视频标题，避免不同任务覆盖。
